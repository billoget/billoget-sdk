...
export class UBSService {
   ...
   */
  async getPublicBudget(id: number): Promise<PublicBudget | null> {
    try {
      const response = await this.httpClient.get<PublicBudget>(
        `/api/public/budgets/public/${id}`
      );
   ...
   */
  async getBudgetByToken(token: string): Promise<PublicBudget | null> {
    try {
      const response = await this.httpClient.get<PublicBudget>(
        `/api/public/budgets/token/${token}`
      );
      return response.data;
    } catch (error: any) {
      if (error.response?.status === 404) {
  ...
  async debugToken(token: string): Promise<{
    ...
    }>(`/api/public/budgets/debug/token/${token}`);
    return response.data;
  }
   ...
   */
  async confirmBudget(token: string): Promise<{
    message: string;
    budget?: PublicBudget;
    paymentUrl?: string;
  }> {
    const response = await this.httpClient.post<{
      ...
      budget?: PublicBudget;
      paymentUrl?: string;
    }>(`/api/public/budgets/token/${token}/confirm`);
    return response.data;
  }
   ...
   */
  async rejectBudget(token: string): Promise<{
    message: string;
    budget?: PublicBudget;
  }> {
    const response = await this.httpClient.post<{
      message: string;
      budget?: PublicBudget;
    }>(`/api/public/budgets/token/${token}/reject`);
    return response.data;
  }
